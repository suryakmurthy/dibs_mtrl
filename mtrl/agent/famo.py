...
class Agent(AbstractAgent):
    ...
    def __init__(
        ...
        self.critic_weights = torch.tensor([0.0] * self.num_tasks, device=device, requires_grad=True)
        self.critic_weights_opt = torch.optim.Adam([self.critic_weights], lr=w_lr, weight_decay=gamma)
    ...
    def train(self, training: bool = True) -> None:
        ...
        for name, component in self._components.items():
            if name not in [ "log_alpha", "critic_weights"]:
    ...
    def update_critic(
        ...
        weights = F.softmax(self.critic_weights, -1)
        c = (weights / (task_loss + 1e-8)).sum().detach()
        real_backprop_loss = torch.sum(weights * (task_loss + 1e-8).log() * c)
        ...
        ### update critic task weights
        with torch.no_grad():
            current_Q1_, current_Q2_= self.critic(
                mtobs=mtobs,
                action=batch.action,
                detach_encoder=False,
            )
            critic_loss_ = F.mse_loss(
                current_Q1_, target_Q, reduction=self.loss_reduction
            ) + F.mse_loss(current_Q2_, target_Q, reduction=self.loss_reduction)

            task_loss_ = self._convert_loss_into_task_loss(
                loss=critic_loss_, env_metadata=kwargs_to_compute_gradient["env_metadata"]
            ).view(-1) # (num_tasks,)

            self.critic_weights_opt.zero_grad()
            with torch.enable_grad():
                #delta = torch.clip(task_loss, 1e-4).log() - torch.clip(task_loss_, 1e-4).log()
                delta = (task_loss + 1e-8).log() - (task_loss_ + 1e-8).log()
                d = torch.autograd.grad(
                    F.softmax(self.critic_weights, -1), self.critic_weights, grad_outputs=delta.detach())[0]
            self.critic_weights.grad = d
            self.critic_weights_opt.step()
